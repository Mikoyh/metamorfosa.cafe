...
const HomePage: React.FC<HomePageProps> = ({ setPage, user, isLoggedIn, onLoginClick, userOrder, allActiveOrders, leaderboard, userHistory, onAddToCart, onProductClick, wallNotes, isBirthday, isCafeOpen, isGoFoodOpen, currentTime, party, availableVouchers }) => {
  ...
  return (
    <div className="lg:grid lg:grid-cols-2 lg:gap-8 lg:items-start pb-36 pt-20 lg:pt-8 px-4 space-y-6 lg:space-y-0 bg-slate-50/50">
      ...
      <div className="space-y-6">
        ...
        {isLoggedIn && userHistory.length > 0 && (
           <section>
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0 lg:grid lg:grid-cols-2">
                  {userHistory.slice(0, 4).map((item, i) => (
                      <div key={`hist-${i}`} className="shrink-0 w-32 lg:w-full bg-white rounded-[1.5rem] p-3 border border-slate-100 shadow-sm" onClick={() => onProductClick(item)}>
