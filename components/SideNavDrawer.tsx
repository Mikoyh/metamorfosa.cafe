...
const NavContent: React.FC<Omit<SideNavDrawerProps, 'isOpen'>> = ({ onClose, user, isLoggedIn, onLoginClick, onLogout, setPage, isStaffMode, onStaffAccess, setIsStaffMode }) => {
    const navLinks = [
...
const SideNavDrawer: React.FC<SideNavDrawerProps> = (props) => {
  const { isOpen, onClose } = props;

  return (
    <>
      {/* Desktop (permanent sidebar) */}
      <div className="hidden lg:flex fixed top-0 left-0 bottom-0 w-72 bg-white flex-col shadow-2xl border-r border-slate-100">
         <NavContent {...props} />
      </div>
