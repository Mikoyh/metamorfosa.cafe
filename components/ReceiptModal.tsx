      ...
      {isOpen && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
          ...
          <MotionDiv 
            ...
            className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10 flex flex-col max-h-[85vh]"
          >
            <div className="p-8 pb-0 flex flex-col items-center flex-shrink-0">
            ...
            <div className="overflow-y-auto px-8 no-scrollbar">
                ...
                <div className="space-y-2 mb-6 border-b border-slate-200 pb-4">
                  {order.items.map(item => (
                    <div key={item.id} className="flex justify-between text-xs">
                      <span className="text-slate-600 font-bold flex-1 pr-2">{item.name} <span className="text-slate-400">x{item.quantity}</span></span>
                      <span className="font-black text-slate-800 text-right">Rp {(item.price * item.quantity).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="p-8 pt-6 flex-shrink-0">
              ...
              <button onClick={onClose} className="mt-6 text-xs w-full font-black text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-1">Tutup</button>
            </div>
          </MotionDiv>
        </div>
      )}
