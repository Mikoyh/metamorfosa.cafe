...
const BottomNav: React.FC<BottomNavProps> = ({ page, setPage, isLoggedIn, onLoginClick, isStaffMode, onLogout, onCartClick, cartCount }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-[90px] z-[490] max-w-md mx-auto">
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 pb-safe pt-2 px-2 h-[72px] flex justify-around items-center shadow-[0_-15px_40px_rgba(0,0,0,0.08)]">
        {navItems.map((item, index) => {
          const isActive = page === item.id;
          const isCenter = !isStaffMode && index === 2;
          
          if (isCenter) {
            return <div key={item.id} className="w-20" />; // Placeholder for the center button
          }
      ...
      {!isStaffMode && (
          <div className="absolute top-0 left-1/2 -translate-x-1/2 flex justify-center items-center">
              <motion.div 
                whileTap={{ scale: 0.9 }}
                className={`w-20 h-20 rounded-full flex flex-col items-center justify-center shadow-2xl border-4 border-white bg-[#1b4332] text-white cursor-pointer transition-all`}
