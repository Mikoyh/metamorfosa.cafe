...
export default function App() {
  ...
  const findRandomSpot = () => {
      ...
      // Edge-to-edge placement: x can be from 0 up to (wallWidth - noteWidth)
      const x = Math.random() * (wallWidth - NOTE_WIDTH);
      // y can be from the top to the bottom of the wall container
  ...
  const generateDenseNpcNotes = () => {
      ...
      const noteCount = 120; // Optimal number for performance and density
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const seedNotes = generateDenseNpcNotes();
  ...
  useEffect(() => {
    const isAnyModalOpen = isLoginOpen || isRegisterOpen || isSideNavOpen || !!selectedProduct || isCartOpen || isNotificationOpen || isPaymentModalOpen || isReceiptOpen || isRatingOpen || isStaffPasswordModalOpen;
    if (isAnyModalOpen) {
  ...
  }, [isLoginOpen, isRegisterOpen, isSideNavOpen, selectedProduct, isCartOpen, isNotificationOpen, isPaymentModalOpen, isReceiptOpen, isRatingOpen, isStaffPasswordModalOpen]);
  ...
  const leaderboardData: User[] = useMemo(() => {
    const npcs: User[] = Array.from({length: 100}).map((_, i) => ({
      ...
      bannerId: PROFILE_BANNERS[Math.floor(Math.random() * PROFILE_BANNERS.length)].id,
      frameId: 'default',
      isVerified: i < 5
    }));
  ...
  const addXP = (amount: number) => {
    setUser(prev => {
      ...
      if (XP_FOR_LEVEL[prev.level] && newXp >= XP_FOR_LEVEL[prev.level]) {
        ...
        // Auto-equip new frame on level up
        if (newLevel === 5) newFrameId = 'silver';
        if (newLevel === 10) newFrameId = 'legend';
      }
  ...
  useEffect(() => {
    // This logic now runs inside an effect to prevent infinite loops during render
    const currentUserInLeaderboard = leaderboardData.find(u => u.name === user.name);
    if (currentUserInLeaderboard && user.rank !== currentUserInLeaderboard.rank) {
      setUser(prev => ({ ...prev, rank: currentUserInLeaderboard.rank }));
    }
    ...
    const interval = setInterval(() => {
      ...
      setAllActiveOrders(prev => {
        ...
        const remainingOrders = prev.filter(o => {
          if (o.isNpc && o.status === 'READY' && now - o.timestamp > 40000) {
             ...
             finishedNpcOrders.push(o);
             return false;
          }
          return true;
        });
        
        finishedNpcOrders.forEach(order => {
            if (Math.random() > 0.4) { // 60% chance for NPC to leave a review
                const ratingText = NPC_RATING_NOTE_SAMPLES[Math.floor(Math.random() * NPC_RATING_NOTE_SAMPLES.length)];
                handleAddNote({
                    text: ratingText,
      ...
      if (allActiveOrders.length < 8 && Math.random() > 0.65) {
        ...
        const newNpcOrder: ActiveOrder = {
          ...
          status: 'WAITING', isNpc: true,
          tableNumber: String(Math.floor(Math.random() * 20) + 1),
          timestamp: Date.now(), countdown: 5
        };
        setAllActiveOrders(prev => [...prev, newNpcOrder]);
      }
    }, 8000);
    return () => clearInterval(interval);
  }, [allActiveOrders.length, leaderboardData]);

  useMotionValueEvent(scrollY, "change", (latest) => {
    const direction = latest > lastScrollY.current ? 'down' : 'up';
    if (latest < 50) {
        setIsHeaderVisible(true);
    } else if (direction === 'down' && isHeaderVisible) {
  ...
  const handlePageChange = (newPage: Page, data?: any) => {
    if (newPage === 'profile' && !isLoggedIn && !data?.user) {
      setIsLoginOpen(true);
      return;
    }
  ...
  const handleConfirmPayment = (tableNumber: string) => {
    ...
    const newOrder: ActiveOrder = {
        ...
        user, items: orderToPay.items, tableNumber,
  ...
  const isBirthday = useMemo(() => {
    ...
    // Note: Date month is 0-indexed, so month - 1
    ...
    <div className="max-w-md mx-auto min-h-screen bg-green-50/20 shadow-2xl relative flex flex-col">
      ...
      <main className="flex-grow">
        <AnimatePresence mode="wait">
          {page.name === 'home' && <HomePage key="home" setPage={handlePageChange} user={user} isLoggedIn={isLoggedIn} onLoginClick={() => setIsLoginOpen(true)} userOrder={allActiveOrders.find(o => !o.isNpc && o.user.name === user.name)} allActiveOrders={allActiveOrders} leaderboard={leaderboardData.slice(0, 5)} userHistory={userHistory} onAddToCart={handleAddToCart} onProductClick={setSelectedProduct} wallNotes={wallNotes} isBirthday={isBirthday} />}
          {page.name === 'menu' && <MenuPage key="menu" onProductClick={setSelectedProduct} onAddToCart={handleAddToCart} isHeaderVisible={isHeaderVisible} favorites={user.favorites} toggleFavorite={toggleFavorite} menuOptions={page.data} />}
          ...
          {page.name === 'queue-history' && <QueueHistoryPage key="queue" activeOrders={allActiveOrders} historyOrders={historyOrders} user={user} setPage={handlePageChange} leaderboardData={leaderboardData} />}
        </AnimatePresence>
      </main>
